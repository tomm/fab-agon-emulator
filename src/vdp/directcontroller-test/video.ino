...
{
  auto fgcolor = VGAController.createRawPixel(RGB222(3, 0, 0)); // red
  auto bgcolor = VGAController.createRawPixel(RGB222(0, 0, 2)); // blue
  ...
  for (int i = 0; i < scanlinesPerCallback; ++i) {
    ...
    if (scanLine < borderSize || scanLine > height - borderSize)
      bgcolor = VGAController.createRawPixel(RGB222(fastRandom(), fastRandom(), fastRandom()));
...
void setup()
{
  ...
  VGAController.setDrawScanlineCallback(drawScanline);
...
{
  task_loop {
    // Flush incoming data from ez80
    if (Serial2.available() > 0) {
      Serial2.read();
    }

    // test collision with borders and bounce changing direction
    if (objX < objSize / 2 || objX > VGAController.getScreenWidth() - objSize / 2)
      objDir = M_PI - objDir;
    else if (objY < borderSize + objSize / 2 || objY > VGAController.getScreenHeight() - borderSize - objSize / 2)
      objDir = 2 * M_PI - objDir;
